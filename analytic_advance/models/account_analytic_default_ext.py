...
class AccountAnalyticDefaultExt(models.Model):
    ...
    first_type = fields.Selection(
        [('model', '模型'), ('api', '接口'), ('pre', '上级结果')], string="操作数1类型", default="model")
    ...
    first_field_config_name = fields.Many2one(
        'field.config.table', string='操作数1字段名称')
    field_domain = fields.Char(
        string='范围', compute='_compute_field_domain',
        readonly=False, store=True)

    second_name = fields.Char(string='操作数2名称')
    second_type = fields.Selection(
        [('model', '模型'), ('api', '接口'), ('pre', '上级结果')], string="操作数2类型", default="model")
    ...
    second_field_config_name = fields.Many2one(
        'field.config.table', string='操作数2字段名称')
    operator = fields.Selection(
        [('add', '＋'), ('reduce', '－'), ('multiply', '×'), ('divide', '÷')], string="操作符")
